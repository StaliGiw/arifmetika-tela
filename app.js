...
function openPatientForm(id=null) { id = typeof id === 'string' ? id : null; editPatientId=id; const p=db.patients.find(x=>x.id===id); const form=document.getElementById('patientForm'); form.reset(); document.getElementById('patientFormMode').textContent=p?'Редактирование карточки':'Новая карточка'; if(p) Object.keys(p).forEach(k=>{if(form.elements[k] && typeof p[k]!=='object') form.elements[k].value=p[k]||'';}); buildCategoryPicker(p?.categories||[]); const dialog=document.getElementById('patientDialog'); if(!dialog.open) dialog.showModal(); setTimeout(()=>form.elements.firstName.focus(),50); }
