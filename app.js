...
function saveDb() {
  ...
  if (window.desktopDb) window.desktopDb.save(db).catch(() => notify('Не удалось записать файл базы'));
  document.querySelector('.save-state').classList.add('pulse');
  setTimeout(() => document.querySelector('.save-state').classList.remove('pulse'), 500);
}
async function hydrateDb() {
  if (!window.desktopDb) return;
  try {
    const saved = await window.desktopDb.load();
    if (saved && Array.isArray(saved.patients)) db = saved;
    else await window.desktopDb.save(db);
    renderPatients(); renderStats();
  } catch { notify('База открыта в резервном режиме'); }
}
...
function renderPatientDetail() {
  ...
  if (!p) { panel.className = 'patient-detail glass empty-state'; panel.innerHTML = `<div><span class="empty-icon">♙</span><h2>Выберите пациента</h2><p>Или создайте первую карточку — достаточно указать имя.</p><button class="primary" id="emptyAdd">＋ Новый пациент</button></div>`; document.getElementById('emptyAdd').onclick = () => openPatientForm(null); return; }
  panel.className = 'patient-detail glass'; const visits = [...(p.visits || [])].sort((a,b) => b.date.localeCompare(a.date));
  panel.innerHTML = `<div class="patient-title"><span class="avatar large">${esc(initials(p))}</span><div><h2>${esc(fullName(p))}</h2><div class="tag-row">${(p.categories || []).map(c => `<span>${esc(c)}</span>`).join('')}</div></div><div class="patient-actions"><button class="secondary" id="editPatient">✎ Редактировать</button><button class="danger" id="deletePatient">Удалить</button></div></div><div class="info-grid"><article class="info-card blue-info"><h3>Основные данные</h3><dl><div><dt>Телефон</dt><dd>${esc(p.phone || 'Не указан')}</dd></div><div><dt>Дата рождения</dt><dd>${formatDate(p.birthDate)}</dd></div></dl></article><article class="info-card green-info"><h3>Жалобы и состояние</h3><p>${esc(p.complaints || p.diagnosis || 'Информация пока не добавлена.')}</p></article><article class="info-card red-info"><h3>Важно</h3><p>${esc(p.warnings || 'Противопоказания и ограничения не указаны.')}</p></article></div><section class="history"><div class="card-title"><div><h3>История приёмов</h3><small>${visits.length} ${visits.length === 1 ? 'приём' : 'приёмов'}</small></div><button class="primary" id="addVisit">＋ Добавить приём</button></div><div class="visit-list">${visits.length ? visits.map(v => `<article class="visit-row"><time>${formatDate(v.date)}<small>${esc(v.type)}</small></time><div><strong>${esc([v.massageType, v.massageArea].filter(Boolean).join(" · ") || v.procedure)}</strong><p>${esc(v.procedure)}${v.result || v.recommendations ? " · " : ""}${esc(v.result || v.recommendations || 'Без дополнительных заметок')}</p></div><div class="visit-price"><strong>${AppCalculations.currency(v.price)}</strong><small class="${v.paymentStatus}">${v.paymentStatus === 'paid' ? 'Оплачено' : v.paymentStatus === 'partial' ? 'Частично' : 'Не оплачено'}</small><button class="visit-delete" data-delete-visit="${v.id}" title="Удалить приём">×</button></div></article>`).join('') : `<div class="history-empty">Приёмов пока нет. Добавьте первый проведённый сеанс.</div>`}</div></section>`;
  document.getElementById('editPatient').onclick = () => openPatientForm(p.id); document.getElementById('deletePatient').onclick = () => deletePatient(p.id); document.getElementById('addVisit').onclick = openVisitForm; panel.querySelectorAll('[data-delete-visit]').forEach(button => button.onclick = () => deleteVisit(p.id, button.dataset.deleteVisit));
}
...
function openPatientForm(id=null) { id = typeof id === 'string' ? id : null; editPatientId=id; const p=db.patients.find(x=>x.id===id); const form=document.getElementById('patientForm'); form.reset(); document.getElementById('firstVisitSection').classList.toggle('hidden', Boolean(p)); form.elements.firstVisitDate.value=today(); form.elements.firstDuration.value=60; form.elements.firstPrice.value=2000; form.elements.firstPaidAmount.value=2000; document.getElementById('patientFormMode').textContent=p?'Редактирование карточки':'Новая карточка'; if(p) Object.keys(p).forEach(k=>{if(form.elements[k] && typeof p[k]!=='object') form.elements[k].value=p[k]||'';}); buildCategoryPicker(p?.categories||[]); const dialog=document.getElementById('patientDialog'); if(!dialog.open) dialog.showModal(); setTimeout(()=>form.elements.firstName.focus(),50); }
['addPatient','quickAdd'].forEach(id => document.getElementById(id).onclick = () => openPatientForm());
document.getElementById('patientForm').addEventListener('submit', e => { e.preventDefault(); const form=e.currentTarget; const raw=Object.fromEntries(new FormData(form)); const categories=[...document.querySelectorAll('#categoryPicker input:checked')].map(x=>x.value); const patientData={}; ['lastName','firstName','middleName','birthDate','phone','secondPhone','complaints','diagnosis','warnings','notes'].forEach(key=>patientData[key]=raw[key]||''); if(editPatientId){const current=db.patients.find(p=>p.id===editPatientId); Object.assign(current,patientData,{categories}); selectedPatientId=current.id;} else {const visits=[]; if(raw.firstMassageType || raw.firstProcedure){visits.push({id:uid(),date:raw.firstVisitDate||today(),type:'Первичный',massageType:raw.firstMassageType||'',massageArea:raw.firstMassageArea||'',duration:Number(raw.firstDuration)||0,price:Number(raw.firstPrice)||0,paidAmount:Number(raw.firstPaidAmount)||0,paymentStatus:Number(raw.firstPaidAmount)>=Number(raw.firstPrice)?'paid':Number(raw.firstPaidAmount)>0?'partial':'unpaid',procedure:raw.firstProcedure||raw.firstMassageType,result:'',recommendations:''});} const patient={...patientData,id:uid(),categories,visits,createdAt:new Date().toISOString()}; db.patients.push(patient); selectedPatientId=patient.id;} saveDb(); document.getElementById('patientDialog').close(); route('patients'); notify('Карточка пациента сохранена'); });

function deletePatient(id) { const p=db.patients.find(x=>x.id===id); if(!p || !confirm('Удалить пациента «'+fullName(p)+'» и всю историю приёмов? Отменить это действие будет нельзя.')) return; db.patients=db.patients.filter(x=>x.id!==id); selectedPatientId=null; saveDb(); renderPatients(); renderStats(); notify('Пациент удалён'); }
function deleteVisit(patientId, visitId) { const p=db.patients.find(x=>x.id===patientId); if(!p || !confirm('Удалить этот приём из истории пациента?')) return; p.visits=(p.visits||[]).filter(v=>v.id!==visitId); saveDb(); renderPatients(); renderStats(); notify('Приём удалён'); }
